...
function mockPrivyFetch(privateKey: Uint8Array, sigStructure: Uint8Array): typeof fetch {
	return vi.fn(async () => {
...
describe('handleSign', () => {
	...
	it('rejects bad bearer with 401', async () => {
		...
		const response = await handleSign(
			signRequest({ privateKey, bearer: 'wrong-token' }),
			{ env, fetchImpl: mockPrivyFetch(privateKey, sigStructure) }
		);
	...
	it('rejects unknown keyRef with 404', async () => {
		...
		const sigStructure = new Uint8Array([1, 2, 3, 4]);
		const response = await handleSign(
			signRequest({ privateKey, keyRef: 'missing-key' }),
			{ env, fetchImpl: mockPrivyFetch(privateKey, sigStructure) }
		);
	...
	it('rejects unknown logId with 404', async () => {
		...
		const sigStructure = new Uint8Array([1, 2, 3, 4]);
		const response = await handleSign(
			signRequest({ privateKey, logId: 'f'.repeat(32) }),
			{ env, fetchImpl: mockPrivyFetch(privateKey, sigStructure) }
		);
		expect(response.status).toBe(404);
	});
