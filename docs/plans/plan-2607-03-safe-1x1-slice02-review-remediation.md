...
**Status:** IMPLEMENTED (2026-07-29 — R1 + M2 + M3 applied on the PR branch;
R2 remains FOR-504, sibling repo)
**Date:** 2026-07-29
...
- **R1 (M1)**: `handle-delegation-required` catches
  `InteractiveRootSignerError` → outcome log `'interactive_root'`,
  reservation cleared, **200 + `ok: false`**. The coordinator retries every
  non-2xx through the full ladder, so acknowledgement — not a 4xx — is the
  non-retryable shape; the demand stays in the pending queue. Unit test
  added.
- **M2**: attestation now arms from `MANDATE_SIGNER_URL` env as well as
  flags (partial config errors loudly instead of silently posting
  unattested); CLI source-assertion test added
  (`onboard-request.cli.test.ts`).
- **M3**: unknown `--mode` errors loudly instead of coercing to
  Privy-custody (Mode C).
